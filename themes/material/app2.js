...
if (UI.dark_mode) {
  document.write(`<style>* {box-sizing: border-box}body{color:rgba(255,255,255,.87);background-color:#333232}.mdui-theme-primary-${UI.main_color} .mdui-color-theme{background-color:#232427!important}.mdui-textfield-input,.mdui-textfield-label{color:#fff!important;}</style>`);
}
...
function title(path) {
  ...
  if (model.is_search_page)
    $('title').html(`${document.siteName} - ${drive_name} - ${model.q}`);
  else
...
function append_files_to_list(path, files) {
  ...
  for (i in files) {
    ...
    if (item['mimeType'] == 'application/vnd.google-apps.folder') {
      ...
      if ("|html|php|css|go|java|js|json|txt|sh|md|mp4|webm|avi|bmp|jpg|jpeg|png|gif|m4a|mp3|flac|wav|ogg|".indexOf(`|${ext}|`) >= 0) {
